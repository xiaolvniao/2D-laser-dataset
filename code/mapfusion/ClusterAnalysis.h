...
private:
    ...
    int dataNum;
    int minPTs;
    int clusterId; //簇的数量（不保含-1噪音簇）
public:
    ClusterAnalysis() {}                //Default constructor.
//    void Init(const char* fileName, float radius, int minPTs);    //Initialization operation
    void Init(float* raw_data,int dime_number,int data_size, float radius, int minPTs);//Initialization operation
