...
//    printf("dataNum is: %d\n", dataNum);
//}
void ClusterAnalysis::Init(float* raw_data,int dime_number,int data_size, float radius, int minPTs)
{
    this->radius = radius;        //set the radius
    this->minPTs = minPTs;        //set the minPTs
    this->clusterId = 0;
...
//    int data_size;
//    printf("START GriT-DBSCAN...\n");
//    printf("loading the data_set...\n");
//    raw_data = read_data(fileName, " ", &dim, &data_size);
    this->raw_data = raw_data;
    this->dataNum = data_size;
//    printf("dimension is: %d\n", dime_number - 1);
//    printf("dataNum is: %d\n", dataNum);
}
...
void ClusterAnalysis::identifyCorePts()
{
    ...
    for (int i = 0; i < gridCells.size(); i++)
    {
        ...
        else
        {
            ...
            {
                int cnt = 0;
                float* point = dataSets[pts[query]].GetDimension();
                for (int neiCell = 0; neiCell < neiGrid[i].size(); neiCell++)
                {
                    ...
                    {
                        float* neipoint = dataSets[neiPts[j]].GetDimension();
...
void ClusterAnalysis::constructGridGraph()
{
    ...
    for (int jt = 0; jt < gridCells.size(); jt++)
    {
        ...
        if (clust >= 0)
        {
            ...
            {
                dataSets[pts[i]].SetClusterId(clust);
            }
        }
    }
}
...
bool ClusterAnalysis::Merge(int a, int b)
{
    ...
    for (int i = 0; i < n1; i++)
    {
        if (dataSets[pts[i]].isCore())
        {
            float* pt = dataSets[pts[i]].GetDimension();
            vector<int>& pts_b = gridCells[b].getPoints();
            for (int j = 0; j < n2; j++)
            {
                if (dataSets[pts_b[j]].isCore() &&
                        sqrDistance_1(pt, dataSets[pts_b[j]].GetDimension()) <= sqr_r)
                {
                    return true;
                }
            }
...
bool ClusterAnalysis::Filter(int a, int b, int* visited_a, int* visited_b, int& query)
{
    ...
    visited_a[query] = 1;
    visited_a[n1] -= 1;
    float* queryPoint = dataSets[coreGrid_a[query]].GetDimension();
    for (int i = 0; i < n2; i++)
    {
        if (visited_b[i] == 0)
        {
            float* point = dataSets[coreGrid_b[i]].GetDimension();
    ...
    for (int i = 0; i < DIME_NUM - 1; i++)
    {
        ...
        pt[i] = a - dataSets[coreGrid_b[q]].GetDimension()[i];
        temp += a * pt[i];
    }
    float theta = 1;
    for (int i = 0; i < n2; i++)
    {
        if (visited_b[i] == 0)
        {
            ...
            for (int j = 0; j < DIME_NUM - 1; j++)
            {
                alpha -= dataSets[coreGrid_b[i]].GetDimension()[j] * pt[j];
            }
    ...
    for (int i = 0; i < n1; i++)
    {
        if (visited_a[i] == 0)
        {
            float* point = dataSets[coreGrid_a[i]].GetDimension();
            ...
            for (int query = 0; query < pts.size(); query++)
            {
                ...
                for (int neiCell = 0; neiCell < neiGrid[i].size(); neiCell++)
                {
                    if (gridCells[neiGrid[i][neiCell]].getCoreNum() > 0)
                    {
                        ...
                        for (int j = 0; j < neiPts.size(); j++)
                        {
                            if (dataSets[neiPts[j]].isCore())
                            {
                                float* neipoint = dataSets[neiPts[j]].GetDimension();
                                if (sqrDistance_1(point, neipoint) <= sqr_r)
                                {
                                    dataSets[pts[query]].SetClusterId(dataSets[neiPts[j]].GetClusterId());
                                    break;
                                }
                            }
                        }
                        if (dataSets[pts[query]].GetClusterId() != -1)
                            break;
                    }
                }
            }
...
void ClusterAnalysis::printMessage()
{
    ...
    int* temp = (int*)calloc(clusterId+1, sizeof(int));
    int noisePts = 0;
    for (int i = 0; i < dataNum; i++)
    {
        if (dataSets[i].GetClusterId() != -1)
        {
            temp[dataSets[i].GetClusterId()] += 1;
        }
        else
        {
            temp[clusterId] += 1;
    ...
    printf("The last cluster is noise(ClusterId:-1)\n");
...
bool ClusterAnalysis::WriteToFile(const char* fileName)
{
    ...
    for (int i = 0; i < dataNum; i++)
    {
        of1 << dataSets[i].GetOldIndex() << " " <<
            dataSets[i].GetClusterId() << " " << dataSets[i].isCore() << endl;
    }
    of1.close();
    return true;
}
